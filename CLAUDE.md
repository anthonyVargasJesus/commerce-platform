...
Retries: service-to-service calls are retried by the resilience handler, POSTs included, so any POST that changes state must be idempotent. Orders sends an `Idempotency-Key` with every Inventory stock adjustment and Inventory records it in `processed_requests` in the same transaction as the change; follow that pattern for new state-changing calls between services. The gateway also applies CORS (`Cors:AllowedOrigins`) and a per-user rate limit (`RateLimiting`).
